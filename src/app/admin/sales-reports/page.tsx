...
export default function SalesReportsPage() {
    ...
    // Data Fetching
    const { data: orders, isLoading: l1 } = useCollection<OrderFS>(useMemoFirebase(() => firestore ? collection(firestore, 'orders') : null, [firestore]));
    const { data: stores, isLoading: l2 } = useCollection<Store>(useMemoFirebase(() => firestore ? collection(firestore, 'stores') : null, [firestore]));
    const { data: provinces, isLoading: l3 } = useCollection<AppProvince>(useMemoFirebase(() => firestore ? collection(firestore, 'app_provinces') : null, [firestore]));
